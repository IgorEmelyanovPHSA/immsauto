...
public class ReceiveSupplies extends BaseTest {
	...
	public void Validate_Receive_Supplies_as_an_PPHIS_BCVAXDEVIT() throws InterruptedException {
		...
		System.out.println("/*-- 1.Login as an PPHIS user to Supply Location on BCVAXDEVIT --*/");
		...
		System.out.println("/*-- 14. Supply Destribution Description is: -->" + supplyDesc);
		Thread.sleep(2000);
		...
		Thread.sleep(2000);
		System.out.println("/*-- 21. Validate Quantity Filed Present on Layout --*/");
		String qty = inClinicExperiencePage.validateQTYField();
		String expectedqtyLabel = "Quantity";
		System.out.println("/*-- 21.1.: -->" + qty + "Field is Present on Layout and is Mandatory");
		Assert.assertEquals((qty), (expectedqtyLabel));
		Thread.sleep(2000);
		System.out.println("/*-- 22. Validate Dose Conversion Factor Filed Present on Layout --*/");
		String dcf = inClinicExperiencePage.validateDCFField();
		String expecteddcfLabel = "Dose Conversion Factor";
		System.out.println("/*-- 22.1.: -->" + dcf + "Field is Present on Layout");
		Assert.assertEquals((dcf), (expecteddcfLabel));
		Thread.sleep(2000);
		System.out.println("/*-- 23. Validate Doses Filed Present on Layout --*/");
		String doses = inClinicExperiencePage.validateDosesField();
		String expecteddosesLabel = "*Doses";
		System.out.println("/*-- 23.1.: -->" + doses + "Field is Present on Layout");
		Assert.assertEquals((doses), (expecteddosesLabel));
		Thread.sleep(2000);
		System.out.println("/*-- 24. Validate Supply Distribution To Filed Present on Layout --*/");
		...
		System.out.println("/*-- 24.1.: -->" + supplydistributionto + "Field is Present on Layout");
		Assert.assertEquals((supplydistributionto), (expectedsupplydistributiontoLabel));
		Thread.sleep(2000);
		System.out.println("/*-- 25. Save Button is Present on Layout --*/");
