...
public class BulkTransfers extends BaseTest {
	...
	public void Can_do_Bulk_transfers_by_Dosages_form_one_Clinic_to_Another_as_PPHIS_BCVAXDEVIT() throws InterruptedException {
		...
		double remainingDoses_before_Lot_EK4241_Distribution_1_2 = supplyConsolePage.getValueOfRemainingDoses_Container3_Distribution_1_1();
		log("/*-- . remaining doses are: -->" + remainingDoses_before_Lot_EK4241_Distribution_1_2);
		Thread.sleep(2000);
		double remainingQty_before_Lot_EK4241_Distribution_1_2 = supplyConsolePage.getValueOfRemainingQty_Container3_Distribution_1_1();
		...
		Thread.sleep(5000);
		log("/*13.----Getting Remaining Doses/Quantity - AFTER - Automation Location_1 --*/");
		log("/*- container#1 -Automation Supply Distribution_1_1 & Pfizer mRNA BNT162b2 - EK4241*/");
		...
		log("/*- container#3 -Automation Supply Distribution_1_2 & Pfizer mRNA BNT162b2 - EK4241*/");
		double remainingDoses_after_Lot_EK4241_Distribution_1_2 = supplyConsolePage.getValueOfRemainingDoses_Container3_Distribution_1_1();
		log("/*-- . remaining doses are: -->" + remainingDoses_after_Lot_EK4241_Distribution_1_2);
		Thread.sleep(2000);
		double remainingQty_after_Lot_EK4241_Distribution_1_2 = supplyConsolePage.getValueOfRemainingQty_Container3_Distribution_1_1();
		log("/*-- . remaining Quantity are: -->" + remainingQty_after_Lot_EK4241_Distribution_1_2);
		log("/*14.----Validate Remaining Doses/Quantities values BEFORE<->AFTER - Automation Location_1 --*/");
		...
		Thread.sleep(5000);
		log("/*16----Getting id for the latest created Transaction Outgoing 'From' and Incoming 'To'--*/");
		log("/*16.1----Get how many Outgoing Transactions 'From' count records --*/");
		...
		log("/*16.2---Get Outgoing Transaction id 'from' --*/");
		...
		log("/*16.3----Click on the latest created Outgoing Transactions --*/");
		supplyConsolePage.clickOnOutgoingTransactions(kk);
		log("/*--transactions record number --*/:" + kk);
		log("/*17.----Close All Tab's --*/");
		supplyConsolePage.closeAutomationLocationTab();
		Thread.sleep(3000);
		log("/*18.----Go to Supply Locations Tab --*/");
		supplyConsolePage.clickSupplyLocationsTab();
		Thread.sleep(2000);
		log("/*19.----Click on Automation Supply Location_2 --*/");
